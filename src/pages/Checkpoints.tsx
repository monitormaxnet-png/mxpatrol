...
type CheckpointForm = { name: string; nfc_tag_id: string; location_lat: string; location_lng: string; patrol_id: string; sort_order: string; site_id: string; data_log_enabled: boolean; data_log_label: string };
...
const emptyForm: CheckpointForm = { name: "", nfc_tag_id: "", location_lat: "", location_lng: "", patrol_id: "", sort_order: "0", site_id: "", data_log_enabled: false, data_log_label: "" };
...
export default function Checkpoints() {
  ...
  const openEdit = (cp: CheckpointRow) => { setEditId(cp.id); setForm({ name: cp.name, nfc_tag_id: cp.nfc_tag_id, location_lat: cp.location_lat?.toString() || "", location_lng: cp.location_lng?.toString() || "", patrol_id: cp.patrol_id || "", sort_order: cp.sort_order?.toString() || "0", site_id: cp.site_id || "", data_log_enabled: cp.data_log_enabled ?? Boolean(cp.data_log_form_id), data_log_label: cp.data_log_label || "" }); setOpen(true); };
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (uid && dup) { toast.error(`This NFC tag is already registered to "${dup.name}"`); setSaving(false); return; }
    } catch (e) { toast.error("Could not verify NFC tag uniqueness: " + errMsg(e, "Unknown error")); setSaving(false); return; }
    const payload = { company_id: profile.company_id, name: form.name, nfc_tag_id: uid, location_lat: form.location_lat ? parseFloat(form.location_lat) : null, location_lng: form.location_lng ? parseFloat(form.location_lng) : null, patrol_id: form.patrol_id || null, sort_order: parseInt(form.sort_order, 10) || 0, site_id: form.site_id || null, data_log_enabled: form.data_log_enabled, data_log_label: form.data_log_enabled ? (form.data_log_label.trim() || "Datalog") : null, data_log_form_id: null };
  ...
  return <SocPageShell title="Checkpoints" subtitle="Manage all NFC checkpoints and their locations" realtime={realtime}>
    <div className="flex flex-col gap-3 rounded-xl border border-white/10 bg-slate-950/72 p-3 shadow-[0_0_30px_rgba(0,0,0,0.22)] xl:flex-row xl:items-center xl:justify-between">
      ...
      {canManage && <Dialog open={open} onOpenChange={setOpen}><DialogTrigger asChild><Button onClick={openCreate} className="bg-emerald-500 text-black hover:bg-emerald-400"><Plus className="h-4 w-4" /> Add Checkpoint</Button></DialogTrigger><DialogContent className="max-h-[90vh] w-[calc(100vw-1rem)] overflow-y-auto border-white/10 bg-[#050914] text-slate-200 sm:max-w-2xl"><DialogHeader><DialogTitle>{editId ? "Edit Checkpoint" : "Create Checkpoint"}</DialogTitle><DialogDescription className="text-slate-400">Create or edit checkpoint metadata. NFC UIDs are assigned only by an authorized MX Patrol device scan.</DialogDescription></DialogHeader><div className="space-y-4"><div className="grid gap-3 sm:grid-cols-2" id="checkpoint-basics-section"><div className="sm:col-span-2"><Label>Name</Label><Input value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} placeholder="Main Entrance Gate" className="border-white/10 bg-black/40" /></div><div><Label>Checkpoint Code</Label><Input value={editId ? editId.slice(0, 8).toUpperCase() : "Generated after save"} readOnly className="border-white/10 bg-black/40 text-slate-500" /></div><div><Label>Type</Label><Input value={form.location_lat && form.location_lng ? "Mapped" : "Unmapped"} readOnly className="border-white/10 bg-black/40 text-slate-500" /></div></div><div className="rounded-xl border border-emerald-400/20 bg-emerald-400/10 p-3"><Label>NFC Assignment</Label><p className="mt-1 text-xs text-slate-300">{editId && normalizeNfcUid(form.nfc_tag_id) ? "Assigned. Use Replace NFC Tag to change it." : "Pending physical scanner assignment. Select Register Checkpoint in WhatsApp/Web AI and choose the authorized MX Patrol device."}</p></div><div className="grid gap-3 sm:grid-cols-2"><div><Label>Latitude</Label><Input type="number" step="any" value={form.location_lat} onChange={(e) => setForm({ ...form, location_lat: e.target.value })} className="border-white/10 bg-black/40" /></div><div><Label>Longitude</Label><Input type="number" step="any" value={form.location_lng} onChange={(e) => setForm({ ...form, location_lng: e.target.value })} className="border-white/10 bg-black/40" /></div><div><Label>Site</Label><Select value={form.site_id || "none"} onValueChange={(v) => setForm({ ...form, site_id: v === "none" ? "" : v })}><SelectTrigger className="border-white/10 bg-black/40"><SelectValue placeholder="Select site" /></SelectTrigger><SelectContent><SelectItem value="none">Unassigned</SelectItem>{sites.map((s) => <SelectItem key={s.id} value={s.id}>{s.name}</SelectItem>)}</SelectContent></Select></div><div><Label>Patrol Route</Label><Select value={form.patrol_id || "none"} onValueChange={(v) => setForm({ ...form, patrol_id: v === "none" ? "" : v })}><SelectTrigger className="border-white/10 bg-black/40"><SelectValue placeholder="Select route" /></SelectTrigger><SelectContent><SelectItem value="none">None</SelectItem>{patrols.map((p) => <SelectItem key={p.id} value={p.id}>{p.name}</SelectItem>)}</SelectContent></Select></div></div><div className="grid gap-3 sm:grid-cols-2"><div><Label>Enable Datalog?</Label><Select value={form.data_log_enabled ? "yes" : "no"} onValueChange={(v) => setForm({ ...form, data_log_enabled: v === "yes" })}><SelectTrigger className="border-white/10 bg-black/40"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="no">No</SelectItem><SelectItem value="yes">Yes</SelectItem></SelectContent></Select></div>{form.data_log_enabled && <div><Label>Datalog Label</Label><Input value={form.data_log_label} onChange={(e) => setForm({ ...form, data_log_label: e.target.value })} placeholder="Meter Reading" className="border-white/10 bg-black/40" /></div>}</div><div><Label>Sort Order</Label><Input type="number" value={form.sort_order} onChange={(e) => setForm({ ...form, sort_order: e.target.value })} className="border-white/10 bg-black/40" /></div><div className="flex gap-2 border-t border-white/10 pt-3"><DialogClose asChild><Button type="button" variant="outline" className="flex-1" disabled={saving}>Cancel</Button></DialogClose><Button onClick={handleSave} disabled={saving || !form.name.trim()} className="flex-1 bg-emerald-500 text-black hover:bg-emerald-400">{saving && <Loader2 className="mr-2 h-4 w-4 animate-spin" />}{editId ? "Update Checkpoint" : "Create Checkpoint"}</Button></div></div></DialogContent></Dialog>}
      </div>
    </div>

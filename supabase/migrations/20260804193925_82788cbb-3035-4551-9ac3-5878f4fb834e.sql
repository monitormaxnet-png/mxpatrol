-- 1. Idempotency support on the canonical scan table
ALTER TABLE public.scan_logs ADD COLUMN IF NOT EXISTS client_scan_id text;
CREATE UNIQUE INDEX IF NOT EXISTS scan_logs_company_client_scan_id_key
  ON public.scan_logs (company_id, client_scan_id)
  WHERE client_scan_id IS NOT NULL;

-- 2. Authoritative scan -> patrol matching / auto start / auto complete
...
BEGIN
  ...
  -- Candidate selection: only live occurrences, correct tenant/site/device, inside window + tolerance
  SELECT ps.*,
    CASE
  ...
  -- Next expected checkpoint before this scan is applied
  ...
  -- Automatic patrol start on the first accepted route scan
  ...
  -- Progress + completion (handles one-checkpoint patrols atomically)
...
-- 3. Keep occurrences flowing without relying on someone opening a page
CREATE EXTENSION IF NOT EXISTS pg_cron;

DO $$
BEGIN
  PERFORM cron.unschedule('mxpatrol-generate-patrol-sessions');
EXCEPTION WHEN OTHERS THEN NULL;
END $$;

DO $$
BEGIN
  PERFORM cron.unschedule('mxpatrol-advance-patrol-sessions');
EXCEPTION WHEN OTHERS THEN NULL;
END $$;

SELECT cron.schedule(
  'mxpatrol-generate-patrol-sessions',
  '*/5 * * * *',
  $$SELECT public.generate_due_patrol_sessions(now() + interval '2 hours');$$
);

SELECT cron.schedule(
  'mxpatrol-advance-patrol-sessions',
  '*/5 * * * *',
  $$SELECT public.advance_due_patrol_session_statuses();$$
);
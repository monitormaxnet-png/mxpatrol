...
function RouteMore({ route, onEdit, onDuplicate, onStatus, onDelete }: any) {
  return <DropdownMenu><DropdownMenuTrigger className="icon-btn"><MoreVertical className="h-4 w-4" /></DropdownMenuTrigger><DropdownMenuContent align="end" className="border-white/10 bg-slate-950 text-slate-200"><DropdownMenuItem onClick={() => onEdit(route)}>Edit</DropdownMenuItem><DropdownMenuItem onClick={() => onDuplicate(route)}><Copy className="mr-2 h-4 w-4" />Duplicate</DropdownMenuItem>{route.status === "archived" ? <DropdownMenuItem onClick={() => onStatus(route, "active")}><Play className="mr-2 h-4 w-4" />Restore Route</DropdownMenuItem> : route.status === "paused" ? <DropdownMenuItem onClick={() => onStatus(route, "active")}><Play className="mr-2 h-4 w-4" />Resume</DropdownMenuItem> : <DropdownMenuItem onClick={() => onStatus(route, "paused")}><Pause className="mr-2 h-4 w-4" />Pause</DropdownMenuItem>}{route.status !== "archived" ? <DropdownMenuItem onClick={() => onStatus(route, "archived")}><Archive className="mr-2 h-4 w-4" />Archive</DropdownMenuItem> : null}<DropdownMenuSeparator /><DropdownMenuItem className="text-red-300" onClick={() => onDelete(route)}><Trash2 className="mr-2 h-4 w-4" />Delete</DropdownMenuItem></DropdownMenuContent></DropdownMenu>;
}

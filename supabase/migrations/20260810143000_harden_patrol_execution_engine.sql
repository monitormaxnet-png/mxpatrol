...
-- Production hardening addendum: keep the canonical execution layer safe when earlier migrations were partial.
ALTER TABLE public.patrol_sessions
  ADD COLUMN IF NOT EXISTS latest_sos_at timestamptz;
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE n.nspname = 'public' AND c.relname = 'idx_patrol_sessions_schedule_start_unique'
  ) THEN
    IF NOT EXISTS (
      SELECT 1
      ...
      CREATE UNIQUE INDEX idx_patrol_sessions_schedule_start_unique
...
BEGIN
  FOR policy_spec IN
    SELECT * FROM (VALUES
      ('patrol_templates', 'patrol_templates_manage_company'),
      ('patrol_routes', 'patrol_routes_manage_company'),
      ('patrol_route_checkpoints', 'patrol_route_checkpoints_manage_company'),
      ('patrol_schedules', 'patrol_schedules_manage_company'),
      ('patrol_sessions', 'patrol_sessions_manage_company'),
      ('patrol_session_checkpoints', 'patrol_session_checkpoints_manage_company')
  ...
  LOOP
    IF NOT EXISTS (
      ...
      EXECUTE format(
        'CREATE POLICY %I ON public.%I FOR ALL TO authenticated USING (company_id IN (SELECT company_id FROM public.profiles WHERE id = auth.uid())) WITH CHECK (company_id IN (SELECT company_id FROM public.profiles WHERE id = auth.uid()))',
...
ALTER TABLE public.patrol_sessions REPLICA IDENTITY FULL;
ALTER TABLE public.patrol_session_checkpoints REPLICA IDENTITY FULL;
ALTER TABLE public.incidents REPLICA IDENTITY FULL;
ALTER TABLE public.alerts REPLICA IDENTITY FULL;

ALTER VIEW public.patrol_session_reports SET (security_invoker = true);
GRANT SELECT ON public.patrol_session_reports TO authenticated;

-- Duplicate-safe generator override. It does not depend solely on a unique index being present.
CREATE OR REPLACE FUNCTION public.generate_due_patrol_sessions(p_until timestamptz DEFAULT now() + interval '24 hours')
RETURNS integer
LANGUAGE plpgsql
SECURITY DEFINER
  ...
  LOOP
    ...
    WHILE session_start IS NOT NULL AND session_start <= p_until LOOP
      ...
      SELECT
        ...
        CASE WHEN session_start <= now() THEN 'awaiting_start' ELSE 'scheduled' END,
        session_start, session_end, COALESCE(route_total, 0), 0, 0, COALESCE(route_total, 0), 0, 0,
        jsonb_build_object('generated_by', 'generate_due_patrol_sessions', 'generated_at', now())
      WHERE NOT EXISTS (
        SELECT 1 FROM public.patrol_sessions existing
        WHERE existing.schedule_id = schedule_row.id
          AND existing.scheduled_start = session_start
      )
      ON CONFLICT DO NOTHING
      RETURNING id INTO session_id;
...
REVOKE ALL ON FUNCTION public.find_eligible_patrol_session_for_event(uuid, uuid, text, timestamp with time zone, uuid) FROM PUBLIC, anon, authenticated;
REVOKE ALL ON FUNCTION public.finalize_patrol_session(uuid) FROM PUBLIC, anon, authenticated;
REVOKE ALL ON FUNCTION public.refresh_patrol_session_event_counts(uuid) FROM PUBLIC, anon, authenticated;
...
GRANT EXECUTE ON FUNCTION public.find_eligible_patrol_session_for_event(uuid, uuid, text, timestamp with time zone, uuid) TO service_role;
GRANT EXECUTE ON FUNCTION public.finalize_patrol_session(uuid) TO service_role;
GRANT EXECUTE ON FUNCTION public.refresh_patrol_session_event_counts(uuid) TO service_role;

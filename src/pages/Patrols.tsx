...
export default function Patrols() {
  ...
  const allRoutes = usePatrolRoutes('all');
  ...
  const allRouteRows = useMemo(() => allRoutes.data ?? [], [allRoutes.data]);
  ...
  const openScheduleEditor = (session: any) => {
    const schedule = scheduleRows.find((row) => row.id === session.schedule_id || row.id === session.patrol_schedules?.id);
    if (schedule) {
  ...
  return (
    <SocPageShell title="Patrols" subtitle="Monitor today's scheduled patrols, active sessions and compliance." realtime={realtime}>
      ...
      {tab === 'validation' ? (
        ...
        <OperationsView loading={sessions.isLoading} sessions={filteredSessions} activeSessions={activeSessions} attentionItems={attentionItems} recentActivity={recentActivity} selectedSession={selectedSession} devices={deviceRows} hasSetupData={templateRows.length > 0 || routeRows.length > 0 || scheduleRows.length > 0} onEditSchedule={openScheduleEditor} onOpenTemplates={() => setTab('templates')} onOpenRoutes={() => setTab('routes')} onOpenSchedules={() => setTab('schedules')} />
      ) : (
        <ConfigurationView tab={tab as Exclude<Tab, 'operations' | 'validation'>} siteId={siteId} templates={templateRows} routes={routeRows} scheduleRoutes={allRouteRows} schedules={scheduleRows} devices={deviceRows} checkpointOptions={checkpointOptions.data ?? []} loading={templates.isLoading || routes.isLoading || allRoutes.isLoading || schedules.isLoading || checkpointOptions.isLoading} generatePending={generate.isPending} createRoutePending={createRoute.isPending} createTemplatePending={createTemplate.isPending} createSchedulePending={createSchedule.isPending} onGenerate={() => generate.mutate()} onCreateRoute={(route) => createRoute.mutate(route)} onCreateTemplate={(template) => createTemplate.mutate(template)} onCreateSchedule={(schedule) => createSchedule.mutate(schedule)} />
      )}
      {editingSchedule ? <EditPatrolDialog kind="schedule" row={editingSchedule} routes={routeRows} templates={templateRows} devices={deviceRows} pending={updateSchedule.isPending} onClose={() => setEditingSchedule(null)} onSave={(values) => updateSchedule.mutate({ id: editingSchedule.id, values }, { onSuccess: () => setEditingSchedule(null) })} /> : null}
    </SocPageShell>
  );
}

function OperationsView({ loading, sessions, activeSessions, attentionItems, recentActivity, selectedSession, devices, hasSetupData, onEditSchedule, onOpenTemplates, onOpenRoutes, onOpenSchedules }: { loading: boolean; sessions: any[]; activeSessions: any[]; attentionItems: ReturnType<typeof buildAttentionItems>; recentActivity: ReturnType<typeof buildRecentActivity>; selectedSession: any; devices: any[]; hasSetupData: boolean; onEditSchedule: (session: any) => void; onOpenTemplates: () => void; onOpenRoutes: () => void; onOpenSchedules: () => void; }) {
...
function ConfigurationView({ tab, siteId, templates, routes, scheduleRoutes, schedules, devices, checkpointOptions, loading, generatePending, createRoutePending, createTemplatePending, createSchedulePending, onGenerate, onCreateRoute, onCreateTemplate, onCreateSchedule }: { tab: Exclude<Tab, 'operations'>; siteId: string; templates: any[]; routes: any[]; scheduleRoutes: any[]; schedules: any[]; devices: any[]; checkpointOptions: CheckpointOption[]; loading: boolean; generatePending: boolean; createRoutePending: boolean; createTemplatePending: boolean; createSchedulePending: boolean; onGenerate: () => void; onCreateRoute: (route: CreatePatrolRouteInput) => void; onCreateTemplate: (template: CreatePatrolTemplateInput) => void; onCreateSchedule: (schedule: CreatePatrolScheduleInput) => void; }) {

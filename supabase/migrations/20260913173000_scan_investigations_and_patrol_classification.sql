...
BEGIN
  ...
  INSERT INTO public.scan_investigations (
    ...
    COALESCE(scan_row.tag_uid, scan_row.nfc_tag_id), scan_row.id, scan_row.scanned_at,
  ...
  IF NOT FOUND OR scan_row.checkpoint_id IS NULL THEN
    IF FOUND THEN
      PERFORM public.create_scan_investigation_for_log(
        p_scan_log_id,
        CASE WHEN COALESCE(scan_row.tag_uid, scan_row.nfc_tag_id) IS NULL THEN 'unknown_uid' ELSE 'unregistered_checkpoint' END,
